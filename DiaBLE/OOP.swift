...
extension MainDelegate {
    ...
    func post(_ endpoint: String, _ jsonObject: Any) async throws -> Any {
        ...
        do {
            log("OOP: posting \(jsonData!.string) to \(request.url!.absoluteString)")
            ...
            do {
                ...
                log("OOP: error while decoding response: \(error.localizedDescription)")
...
extension Sensor {
    ...
    func testOOPActivation() async {
        ...
        log("OOP: sending sensor data to \(server.siteURL)/\(server.activationEndpoint!)...")
        do {
